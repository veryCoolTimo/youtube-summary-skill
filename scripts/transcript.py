...
DEFAULT_LANGS = ["ru", "en"]
_EXPAND = {"en": ["en", "en-US", "en-GB"]}


def get_captions(vid: str, priority=None):
    try:
    ...
    # str(): YAML 1.1 parses unquoted `no` (Norwegian) as False
    attempts = [_EXPAND.get(str(l), [str(l)]) for l in (priority or DEFAULT_LANGS)] + [None]
...
def get_transcript(url: str, cfg: dict):
    vid = video_id(url)
    priority = (cfg.get("captions") or {}).get("languages")
    segs, lang = get_captions(vid, priority) if vid else (None, None)
    if segs:
